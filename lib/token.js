...
token.handle = function (req, res, next) {
	...
	console.log('REQ: '+ req.oauth.grantType);
	console.log('REGEX: '+ this.regex.grantType);
...
token.grant = function (req, res, next) {
	...
	switch (req.oauth.grantType) {
		...
		case 'client_credentials':
			console.log('UUID: '+JSON.stringify(req.query));
			// we have already validated the client id and secret
			if (!req.user)
				req.user = {}
			req.user['id'] = req.query.id;//req.params.uuid;
			token.grantAccessToken.call(oauth, req, res, next);
			return;
...
token.grantAccessToken = function (req, res, next) {
	...
	var createAccessToken = function (err, accessToken) {
		...
		var expires = null;
		oauth.model.getAccessTokenLifetime(req.oauth.client.client_id,oauth.grantType,function(tokenLifetime){
...
token.issueToken = function (req, res, next) {
	...
	oauth.model.getAccessTokenLifetime(req.oauth.client.client_id,oauth.grantType,function(tokenLifetime){
		if (tokenLifetime !== null) {
			req.oauth.accessToken.expires_in = tokenLifetime;
		}
		// That's it!
