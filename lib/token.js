...
token.handle = function (req, res, next) {
	...
	this.model.getClient(creds.client_id, creds.client_secret, function (err, client) {
		if (err) {
			return next(error(err.type||'client_error', false, err));
		}
...
token.grant = function (req, res, next) {
	...
	switch (req.oauth.grantType) {
		...
		case 'client_credentials':
			return this.model.getUserFromClientCredentials(req, req.oauth.client.client_id, function (err, user) {
				if (err) {
					return next(error('server_error', false, err));
				}
...
token.grantAccessToken = function (req, res, next) {
	...
	var createAccessToken = function (err, accessToken) {
		...
		var expires = null;
		oauth.model.getAccessTokenLifetime(req.oauth.client.client_id,req.oauth.grantType,function(tokenLifetime){
			if (tokenLifetime !== null) {
				expires = new Date(oauth.now);
				expires.setSeconds(expires.getSeconds() + tokenLifetime);
				req.oauth.accessToken.expires = expires;
			}

			oauth.model.saveAccessToken(req.oauth.accessToken.access_token, req.oauth.client.client_id, req.user.id, expires, function (err) {
				if (err) return next(error('server_error', false, err));
				issueRefreshToken();
			});
		});
...
token.issueToken = function (req, res, next) {
	...
	if (req.oauth.accessToken.expires !== null) {
		req.oauth.accessToken.expires_in = req.oauth.accessToken.expires;
	}
	// That's it!
...
token._generateToken= function (type, req, callback) {
